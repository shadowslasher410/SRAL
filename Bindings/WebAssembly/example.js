...
async function main() {
  ...
  // Fixed: Converted invalid Base 17 radix lookups back to standard Hexadecimal Base 16
  ...
  if ((currentEngineFeatures & sral.SRALSupportedFeatures.SPEECH) !== 0) {
    ...
    if (specificEngineForExTests !== sral.SRALEngines.NONE) {
      ...
      if ((featEx & sral.SRALSupportedFeatures.SPEECH) !== 0) {
        checkSRAL(
          api.speakEx(specificEngineForExTests, "Testing SRAL SpeakEx, not interrupting.", false),
          "speakEx (no interrupt)",
        );
        api.delay(2000);
        checkSRAL(
          api.speakEx(specificEngineForExTests, "Testing SRAL SpeakEx, interrupting.", true),
          "speakEx (interrupt)",
        );
        api.delay(2000);
      } else {
  ...
  if (voiceCount > 0) {
    ...
    voices.forEach((v, i) => {
      // Fixed: Access strings via standard un-nested Emscripten property layout keys safely
      console.log(`  Voice ${i + 1}: ${v.name || "Unknown"} [${v.language || "N/A"}] (${v.gender || "Unknown"}) - Vendor: ${v.vendor || "Generic"}`);
    });
  } else {
  ...
  if ((currentEngineFeatures & sral.SRALSupportedFeatures.SPEAK_TO_MEMORY) !== 0) {
    ...
    // Fixed: Pull buffer layout elements straight out of linear heap via Uint8Array window overlays
    if (pcm && pcm.dataPointer !== 0 && pcm.dataLength > 0) {
      let rawBufferView = new Uint8Array(sral.HEAPU8.buffer, pcm.dataPointer, pcm.dataLength);
      console.log(`[SUCCESS] Generated Heap Buffer Size: ${rawBufferView.byteLength} bytes | Rate: ${pcm.sampleRate} Hz`);
      api.free(pcm.dataPointer); // Avoid sandboxed memory leaks
    } else {
  ...
  console.log(`Global platform Text-to-Speech synthesizer registry filter mask: 0x${platformTtsMask.toString(16).toUpperCase()}`);
    ...
    if ((currentEngineFeatures & sral.SRALSupportedFeatures.SPEECH) !== 0) {
      ...
      // Fixed: Reordered arguments to pass text first, then the delay timeMs parameter to match C++ contracts
      checkSRAL(
        api.delayOutput(
      ...
      if (specificEngineForExTests !== sral.SRALEngines.NONE) {
        ...
        if ((featEx & sral.SRALSupportedFeatures.SPEECH) !== 0) {
          ...
          // Fixed: Reordered arguments to match the precise underlying native unmanaged function layouts
          checkSRAL(
            api.delayOutputEx(
    ...
    if (api.setEnginesExclude(experimentalExclusionMask)) {
      ...
      api.setEnginesExclude(actualExcludeMask);
    } else {
      console.log(
