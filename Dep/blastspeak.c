...
static int blastspeak_get_stream_format(blastspeak* restrict const instance,
...
static WCHAR* blastspeak_get_wchar_from_utf8(
	...
	WCHAR* const result =
		(WCHAR*)blastspeak_get_temporary_memory(instance, (unsigned int)(max_wchars + 1) * sizeof(WCHAR));
...
int blastspeak_initialize(blastspeak* restrict const instance) {
	...
	static_assert(sizeof(instance->memory_stream_dispids) >= (3 * sizeof(DISPID)),
		"memory_stream_dispids layout tracking shortfall.");
	static_assert(sizeof(instance->voice_token_dispids) >= (2 * sizeof(DISPID)),
		"voice_token_dispids layout tracking shortfall.");
...
static int blastspeak_get_stream_format(blastspeak* restrict const instance,
	...
	if (BS_UNLIKELY(blastspeak_get_long_property(instance, DISPID_WAVE_BitsPerSample, &temp, formatex) == 0 ||
			(temp != 8 && temp != 16))) {
		goto error;
	}
	*bits_per_sample = (unsigned char)temp;

	if (BS_UNLIKELY(blastspeak_get_long_property(instance, DISPID_WAVE_Channels, &temp, formatex) == 0 ||
			(temp != 1 && temp != 2))) {
		goto error;
	}
	*channels = (unsigned char)temp;

	if (BS_UNLIKELY(blastspeak_get_long_property(instance, DISPID_WAVE_SamplesPerSec, &temp, formatex) == 0 ||
			temp < 8000 || temp > 192000)) {
		goto error;
	}
...
char* blastspeak_speak_to_memory(
	...
	*bytes = 0;
	...
	const DISPPARAMS prop_params = {
		.rgvarg = &prop_arg, .rgdispidNamedArgs = (DISPID*)&putref_dispid, .cArgs = 1, .cNamedArgs = 1};
