...
static inline void escape_and_format_direct(char* restrict const dest,
...
static int ring_buffer_push(const char* prefix, const char* text, const char* suffix) {
	...
	while (true) {
		...
		if (atomic_compare_exchange_strong_explicit(
				&g_RingBuffer[slot].state, &expected_empty, SLOT_BUSY, memory_order_acquire, memory_order_relaxed)) {
