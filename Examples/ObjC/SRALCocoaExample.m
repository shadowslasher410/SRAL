...
static SRAL_INLINE void sleep_ms(unsigned int milliseconds) {
  struct timespec ts = {.tv_sec = (time_t)(milliseconds / 1000U),
                        .tv_nsec = (long)((milliseconds % 1000U) * 1000000UL)};
...
static void PrintEngineNames(int engineBitmask, const char *const message_title) {
  ...
  while (mask > 0) {
    const uint32_t lowest_bit = (uint32_t)__builtin_ctz(mask);
...
SRAL_COLD static void print_supported_features(int features) {
  ...
  if (features & SRAL_SUPPORTS_SPEECH) fwrite("  - SUPPORTS_SPEECH\n", 1, 20, stdout);
  if (features & SRAL_SUPPORTS_BRAILLE) fwrite("  - SUPPORTS_BRAILLE\n", 1, 21, stdout);
  if (features & SRAL_SUPPORTS_SPEECH_RATE) fwrite("  - SUPPORTS_SPEECH_RATE\n", 1, 25, stdout);
  if (features & SRAL_SUPPORTS_SPEECH_VOLUME) fwrite("  - SUPPORTS_SPEECH_VOLUME\n", 1, 27, stdout);
  if (features & SRAL_SUPPORTS_SELECT_VOICE) fwrite("  - SUPPORTS_SELECT_VOICE\n", 1, 26, stdout);
  if (features & SRAL_SUPPORTS_PAUSE_SPEECH) fwrite("  - SUPPORTS_PAUSE_SPEECH\n", 1, 26, stdout);
  if (features & SRAL_SUPPORTS_SSML) fwrite("  - SUPPORTS_SSML\n", 1, 18, stdout);
  if (features & SRAL_SUPPORTS_SPEAK_TO_MEMORY)
    fwrite("  - SUPPORTS_SPEAK_TO_MEMORY\n", 1, 29, stdout);
  if (features & SRAL_SUPPORTS_SPELLING) fwrite("  - SUPPORTS_SPELLING\n", 1, 22, stdout);
  putchar('\n');
}
...
int main(__unused int argc, __unused const char *argv[]) {
  ...
  @autoreleasepool {
    ...
    CHECK(SRAL_IsInitialized(), "SRAL_IsInitialized correctly returns true after init.",
    ...
    CHECK((tts_engines & at_engines) == 0, "TTS and assistive-tech masks are disjoint.",
    ...
    uint32_t engine_mask = (uint32_t)available_engines;
    while (engine_mask > 0) {
      const uint32_t lowest_bit = (uint32_t)__builtin_ctz(engine_mask);
      const uint32_t e_val = 1U << lowest_bit;
      printf("  - %s: %s\n", SRAL_GetEngineName((int)e_val),
             CategoryName((int)SRAL_GetEngineCategory((int)e_val)));
      engine_mask &= engine_mask - 1;
    }
    putchar('\n');

    const int current_engine_id = SRAL_GetCurrentEngine();
    const char *const currentEngineName = SRAL_GetEngineName(current_engine_id);
    printf("Current Default Engine: %s (0x%X)\n",
           currentEngineName ? currentEngineName : "None/Unknown", (unsigned int)current_engine_id);

    const char enum_members_msg[] =
        "\nNames of all SRAL_Engines enum members (as per SRAL_GetEngineName):\n";
    fwrite(enum_members_msg, 1, sizeof(enum_members_msg) - 1, stdout);
    ...
    if (active_engines != SRAL_ENGINE_NONE) {
      uint32_t active_mask = (uint32_t)active_engines;
      while (active_mask > 0) {
        const uint32_t lowest_bit = (uint32_t)__builtin_ctz(active_mask);
        const uint32_t e_val = 1U << lowest_bit;
        if ((int)e_val != current_engine_id) {
      ...
      if (specific_engine_for_ex_tests == SRAL_ENGINE_NONE) {
        const uint32_t lowest_bit = (uint32_t)__builtin_ctz((uint32_t)active_engines);
        specific_engine_for_ex_tests = (int)(1U << lowest_bit);
      }
    }

    const int specific_features = (specific_engine_for_ex_tests != SRAL_ENGINE_NONE)
                                      ? SRAL_GetEngineFeatures(specific_engine_for_ex_tests)
                                      : 0;
    ...
    if (specific_engine_for_ex_tests != SRAL_ENGINE_NONE) {
      ...
      const char no_ex_engine_msg[] =
          "\nNo specific engine distinct from default (or no active engines) for Ex tests.\n";
      fwrite(no_ex_engine_msg, 1, sizeof(no_ex_engine_msg) - 1, stdout);
    }

    TEST_SECTION("Keyboard Hooks");
    if (SRAL_RegisterKeyboardHooks()) {
      const char hook_success_msg[] = "[SUCCESS] SRAL_RegisterKeyboardHooks registered.\n";
      fwrite(hook_success_msg, 1, sizeof(hook_success_msg) - 1, stdout);
      prompt_user("Keyboard hooks active. Test Ctrl=Interrupt, Shift=Pause/Resume now.");
    } else {
      const char hook_fail_msg[] = "[INFO] SRAL_RegisterKeyboardHooks failed or unsupported on "
                                   "this platform sandbox configuration.\n";
    ...
    printf("Features for Current Default Engine (%s):\n",
           currentEngineName ? currentEngineName : "None");
    ...
    if (current_engine_features & SRAL_SUPPORTS_SPEECH) {
      ...
      CHECK_SRAL(SRAL_Speak("Testing SRAL Speak, not interrupting previous speech.", false),
                 "SRAL_Speak (no interrupt)");
      sleep_ms(2000);
      CHECK_SRAL(SRAL_Speak("Testing SRAL Speak, interrupting previous speech.", true),
                 "SRAL_Speak (interrupt)");
      sleep_ms(2000);

      if (specific_engine_for_ex_tests != SRAL_ENGINE_NONE &&
          (specific_features & SRAL_SUPPORTS_SPEECH)) {
        TEST_SECTION("SRAL_SpeakEx (Specific Engine)");
        CHECK_SRAL(SRAL_SpeakEx(specific_engine_for_ex_tests,
                                "Testing SRAL SpeakEx, not interrupting.", false),
                   "SRAL_SpeakEx (no interrupt)");
        sleep_ms(2000);
        CHECK_SRAL(
            SRAL_SpeakEx(specific_engine_for_ex_tests, "Testing SRAL SpeakEx, interrupting.", true),
    ...
    if (current_engine_features & SRAL_SUPPORTS_SSML) {
      ...
      if (specific_engine_for_ex_tests != SRAL_ENGINE_NONE &&
          (specific_features & SRAL_SUPPORTS_SSML)) {
        TEST_SECTION("SRAL_SpeakSsmlEx (Specific Engine)");
        CHECK_SRAL(SRAL_SpeakSsmlEx(specific_engine_for_ex_tests, ssml_test, true),
                   "SRAL_SpeakSsmlEx");
        sleep_ms(3000);
      }
    }
    ...
    if (current_engine_features & SRAL_SUPPORTS_SPEAK_TO_MEMORY) {
      ...
      void *const pcm_buffer =
          SRAL_SpeakToMemory("Testing speak to memory audio synthesis.", &buffer_size, &channels,
                             &sample_rate, &bits_per_sample);
      if (pcm_buffer) {
        const char speak_mem_msg[] = "[SUCCESS] SRAL_SpeakToMemory successful.\n";
        fwrite(speak_mem_msg, 1, sizeof(speak_mem_msg) - 1, stdout);
        printf("  Buffer Size: %" PRIu64 " bytes\n"
               "  Channels: %d, Sample Rate: %d Hz, Bits: %d\n",
               buffer_size, channels, sample_rate, bits_per_sample);
        SRAL_free(pcm_buffer);
      }

      if (specific_engine_for_ex_tests != SRAL_ENGINE_NONE &&
          (specific_features & SRAL_SUPPORTS_SPEAK_TO_MEMORY)) {
        TEST_SECTION("SRAL_SpeakToMemoryEx (Specific Engine)");
        void *const pcm_buffer_ex =
            SRAL_SpeakToMemoryEx(specific_engine_for_ex_tests, "Testing speak to memory ex.",
                                 &buffer_size, &channels, &sample_rate, &bits_per_sample);
        if (pcm_buffer_ex) {
          const char speak_mem_ex_msg[] = "[SUCCESS] SRAL_SpeakToMemoryEx successful.\n";
          fwrite(speak_mem_ex_msg, 1, sizeof(speak_mem_ex_msg) - 1, stdout);
    ...
    if (current_engine_features & SRAL_SUPPORTS_BRAILLE) {
      ...
      if (specific_engine_for_ex_tests != SRAL_ENGINE_NONE &&
          (specific_features & SRAL_SUPPORTS_BRAILLE)) {
        TEST_SECTION("SRAL_BrailleEx (Specific Engine)");
        CHECK_SRAL(SRAL_BrailleEx(specific_engine_for_ex_tests, "Testing SRAL Braille Ex output."),
                   "SRAL_BrailleEx");
      }
    }

    TEST_SECTION("SRAL_Output (Default Engine)");
    CHECK_SRAL(SRAL_Output("Testing SRAL Output, not interrupting.", false),
               "SRAL_Output (no interrupt)");
    sleep_ms(2000);
    CHECK_SRAL(SRAL_Output("Testing SRAL Output, interrupting now.", true),
               "SRAL_Output (interrupt)");
    sleep_ms(2000);
    ...
    if (current_engine_features & SRAL_SUPPORTS_SPEECH) {
      ...
      fputs(bool_strings[SRAL_IsSpeaking() ? 1 : 0], stdout);
    ...
    if (current_engine_features & SRAL_SUPPORTS_SPEECH_RATE) {
      ...
      if (SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_SPEECH_RATE, &original_rate)) {
        ...
        if (SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_SPEECH_RATE, &new_rate)) {
          ...
          CHECK(abs(fetched_rate - new_rate) <= 5, "Rate set/get matches close enough",
    ...
    if (current_engine_features & SRAL_SUPPORTS_SPEECH_VOLUME) {
      ...
      if (SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_SPEECH_VOLUME, &original_volume)) {
        ...
        if (SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_SPEECH_VOLUME, &new_volume)) {
          ...
          (void)SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_SPEECH_VOLUME,
                                        &fetched_volume);

          CHECK(abs(fetched_volume - new_volume) <= 5, "Volume set/get matches close enough",
                "Significant volume mismatch");
          (void)SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_SPEECH_VOLUME,
                                        &original_volume);
        }
      }
    }
      ...
      if (SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_VOICE_COUNT, &voice_count) &&
        ...
        if (SRAL_LIKELY(voice_infos != NULL)) {
          if (SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_VOICE_PROPERTIES, voice_infos)) {
            ...
            (void)SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_VOICE_INDEX,
                                          &original_voice_index);

            if (voice_count > 1) {
              const int new_voice_index = (original_voice_index + 1) % voice_count;
              if (SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_VOICE_INDEX,
                                          &new_voice_index)) {
                int current_voice_index = -1;
                (void)SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_VOICE_INDEX,
                                              &current_voice_index);
                CHECK(current_voice_index == new_voice_index, "Voice index tracking matches",
                      "Voice index tracking mismatch");

                if (original_voice_index != -1) {
                  (void)SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_VOICE_INDEX,
                                                &original_voice_index);
    ...
    if (SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_ENABLE_SPELLING,
                                &original_spelling_state)) {
      int new_spelling_state = !original_spelling_state;
      if (SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_ENABLE_SPELLING,
                                  &new_spelling_state)) {
        int spelling_enabled = 0;
        (void)SRAL_GetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_ENABLE_SPELLING,
                                      &spelling_enabled);
        CHECK(spelling_enabled == new_spelling_state, "Spelling state matches",
              "Spelling state mismatch");
        (void)SRAL_SetEngineParameter(SRAL_ENGINE_NONE, SRAL_PARAM_ENABLE_SPELLING,
                                      &original_spelling_state);
      }
    }
    ...
    CHECK(SRAL_SetEnginesExclude(test_exclude_mask), "Excludes successfully configured.",
    ...
    CHECK(test_exclude_mask == new_engines_to_exclude,
          "Excludes configuration verification matches", "Excludes configuration tracking failure");
    ...
    CHECK((current_with_tts_excluded & SRAL_GetTTSEngines()) == 0,
          "No TTS engine is active while excluded.", "TTS leak encountered");
