...
- (void)applicationDidFinishLaunching:(NSNotification*)notification {
	...
	NSButton* speakButton = [NSButton buttonWithTitle:@"Speak" target:self action:@selector(speakClicked:)];
	...
	[self.engineLabel setStringValue:[NSString stringWithFormat:@"Engine: %s", name ? name : "Unknown"]];
...
- (void)updateStatus:(NSTimer*)timer {
	int engine = SRAL_GetCurrentEngine();
	const char* name = SRAL_GetEngineName(engine);
	[self.engineLabel setStringValue:[NSString stringWithFormat:@"Engine: %s", name ? name : "None"]];

	[self.speakingLabel setStringValue:SRAL_IsSpeaking() ? @"Speaking: Yes" : @"Speaking: No"];
}
