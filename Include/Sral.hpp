...
public:
	...
	void Braille(std::string_view text) { Check(SRAL_Braille(text.data()), "Braille output failed"); }

	void Output(std::string_view text, bool interrupt = true) {
		Check(SRAL_Output(text.data(), interrupt), "Output failed");
	}

	void Stop() { SRAL_StopSpeech(); }

	void Pause() { SRAL_PauseSpeech(); }

	void Resume() { SRAL_ResumeSpeech(); }

	[[nodiscard]] bool IsSpeaking() const { return SRAL_IsSpeaking(); }
	...
	[[nodiscard]] int GetCurrentEngineId() const { return SRAL_GetCurrentEngine(); }
	...
	[[nodiscard]] int GetEngineFeatures(int engine_id = 0) const { return SRAL_GetEngineFeatures(engine_id); }
	...
	public:
		...
		void Braille(std::string_view text) { Check(SRAL_BrailleEx(id, text.data()), "Braille output failed"); }

		void Output(std::string_view text, bool interrupt = true) {
			Check(SRAL_OutputEx(id, text.data(), interrupt), "Output failed");
		}

		void Stop() { SRAL_StopSpeechEx(id); }

		void Pause() { SRAL_PauseSpeechEx(id); }

		void Resume() { SRAL_ResumeSpeechEx(id); }

		[[nodiscard]] bool IsSpeaking() const { return SRAL_IsSpeakingEx(id); }
	...
	EngineProxy GetEngine(int engine_id) { return EngineProxy(engine_id, *this); }

	// -------------------------------------------------------------------------
	// Global Hooks & Utils
	// -------------------------------------------------------------------------

	void Delay(int ms) { SRAL_Delay(ms); }

	void RegisterKeyboardHooks() { Check(SRAL_RegisterKeyboardHooks(), "Failed to register keyboard hooks"); }

	void UnregisterKeyboardHooks() { SRAL_UnregisterKeyboardHooks(); }

	[[nodiscard]] int GetAvailableEngines() { return SRAL_GetAvailableEngines(); }

	[[nodiscard]] int GetActiveEngines() { return SRAL_GetActiveEngines(); }
};
