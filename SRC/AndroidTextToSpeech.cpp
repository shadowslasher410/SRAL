...
/**
 * @struct AsyncTtsTask
 * @brief Cache-line isolated tracking structure formatted to mitigate thread bouncing.
 */
struct alignas(destructive_alignment) AsyncTtsTask {
...
struct AndroidTextToSpeech::Impl final {
	...
	alignas(destructive_alignment) std::array<AsyncTtsTask, RING_BUFFER_SIZE> ringQueue;
	alignas(destructive_alignment) std::atomic<size_t> head{0};
	alignas(destructive_alignment) std::atomic<size_t> tail{0};
	alignas(destructive_alignment) std::atomic<bool> ringBell{false};
	alignas(destructive_alignment) std::atomic<bool> stopRequested{false};
...
static bool CheckAndClearException(JNIEnv* env, const char* contextMessage) noexcept {
	if (env->ExceptionCheck()) [[unlikely]] {
		LOGE("JNI Exception intercepted during TTS loop step: %s", contextMessage);
...
bool AndroidTextToSpeech::Impl::PushTask(TaskType type, std::string_view text, float param_val, bool interrupt) noexcept {
...
bool AndroidTextToSpeech::Speak(const char* const speech_text, const bool interrupt) {
	if (!m_impl->initialized.load(std::memory_order_relaxed)) {
		if (!GetActive() && !Initialize()) return false;
	}
...
bool AndroidTextToSpeech::IsSpeaking() {
	...
	ScopedAttachmentGuard jni(m_impl->jvm);
	JNIEnv* const env = jni.GetEnv();
	if (!env || !m_impl->speechObj || !m_impl->midIsSpeaking) return false;

#ifdef __ANDROID__
	const jboolean speaking = env->CallBooleanMethod(m_impl->speechObj, m_impl->midIsSpeaking);
	if (CheckAndClearException(env, "CallBooleanMethod: isSpeaking")) return false;
...
bool AndroidTextToSpeech::GetActive() {
	...
	ScopedAttachmentGuard jni(m_impl->jvm);
	JNIEnv* const env = jni.GetEnv();
	if (!env || !m_impl->speechObj || !m_impl->midIsActive) return false;
...
void AndroidTextToSpeech::Impl::BackgroundWorkerLoop() noexcept {
#ifdef __ANDROID__
	if (!jvm) return;
	JNIEnv* env = nullptr;
	...
	while (!stopRequested.load(std::memory_order_acquire)) {
		...
		AsyncTtsTask& task = ringQueue[current_tail & Impl::RING_MASK];
		...
		if (difference != 0) {
			...
			if (static_cast<intptr_t>(seq) - static_cast<intptr_t>(current_tail + 1) != 0) {
				while (!ringBell.load(std::memory_order_acquire) && !stopRequested.load(std::memory_order_acquire)) {
			...
			if (stopRequested.load(std::memory_order_acquire)) [[unlikely]] break;
			continue;
		}
		...
		if (speechObj) [[likely]] {
			...
			case TaskType::Speak:
				if (midSpeak) {
					...
					if (jtext) {
						...
						CheckAndClearException(env, "CallVoidMethod: speak");
			...
			case TaskType::Stop:
				if (midSilence) {
					...
					CheckAndClearException(env, "CallVoidMethod: stop");
				}
				break;
			case TaskType::SetVolume:
				if (midSetVolume) {
					env->CallVoidMethod(speechObj, midSetVolume, static_cast<jfloat>(param_val));
					CheckAndClearException(env, "CallVoidMethod: setVolume");
				}
				break;
			case TaskType::SetRate:
				if (midSetRate) {
					env->CallVoidMethod(speechObj, midSetRate, static_cast<jfloat>(param_val));
					CheckAndClearException(env, "CallVoidMethod: setSpeechRate");
				}
				break;
			}
		}
#else
		(void)interrupt; (void)param_val;
#endif

		tail.store(current_tail + 1, std::memory_order_release);
		task.sequence.store(current_tail + Impl::RING_BUFFER_SIZE, std::memory_order_release);
	}
