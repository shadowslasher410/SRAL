...
public:
	...
	static bool InitializeInternal(std::shared_ptr<RuntimeContext> ctx) noexcept {
		...
		state.nvdaController_brailleMessage = reinterpret_cast<NVDAController_brailleMessage>(
			::GetProcAddress(state.lib, "nvdaController_brailleMessage"));
...
private:
	static void HandleSpeak(const Command& cmd, SharedState& state) noexcept {
		...
		if (!state.nvdaController_speakText)
			return;
	...
	static void HandlePauseResume(bool pause, SharedState& state) noexcept {
		...
		INPUT inputs[2] = {
			...
			{.type = INPUT_KEYBOARD,
				.ki = {.wVk = VK_SHIFT, .wScan = 0, .dwFlags = KEYEVENTF_KEYUP, .time = 0, .dwExtraInfo = 0}}};
...
public:
	...
	static void UninitializeGlobal() {
		if (!s_context || !s_context->workerThread.joinable())
			return;
